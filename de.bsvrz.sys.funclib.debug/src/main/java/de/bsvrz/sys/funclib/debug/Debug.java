...
/**
 * Stellt Methoden für die Debugausgabe zur Vefügung.<br>
 * Unterstützt werden neben unterschiedlichen DebugLeveln auch hierarisch
 * verwaltete DebugBereiche, die je Klasse angelegt werden können. Für die
 * Ausgabe stehen verschiedene Ausgabekänale mit jeweils spezifischen
 * Formatierungen der Ausgabe zur Verfügung ({@link DebugFormatterXML},
 * {@link DebugFormatterFileText}, {@link DebugFormatterExcel},
 * {@link DebugFormatterHTML}).
 * <p>
 * Die Ausgabe (über den DebugLevel) kann zur Laufzeit sowohl für die
 * verfügbaren Ausgabekanäle als auch die DebugBereiche geändert werden.
 * <p>
 * Für jeden DebugBereich (jeder Klasse, in denen DebugAusgben vorgenommen
 * werden) kann der Level ({@link #ERROR}, {@link #WARNING}, {@link #INFO},
 * {@link #CONFIG}, {@link #FINE}, {@link #FINER}, {@link #FINEST},
 * {@link #ALL}, {@link #OFF}) unabhängig vom Level des jeweiligen Ausgabekanals
 * ({@link DebugFormatterXML}, {@link DebugFormatterStdErrText},
 * {@link DebugFormatterFileText}, {@link DebugFormatterExcel},
 * {@link DebugFormatterHTML}) eingestellt werden. Damit nicht bei einer
 * Applikation mit hunderten von Klassen (und damit entsprechend vielen
 * DebugBereichen) nicht jeden DebugBereich einzeln einstellen zu müssen, werden
 * die DebugBereiche entsprechend ihrer Hierachie verwaltet. Wird der Level
 * eines DebugBereichs geändert, so werden auch alle Level für die
 * untergeordneten Bereiche mit umgestellt. Setzt man also den Level des
 * WurzelLoggers auf FEHLER, so werden alle Logger auf diesen Level gesetzt und
 * nur noch Fehlermeldungen protokolliert. Anschließend kann dann der Logger
 * einer speziel zu untersuchenden Klasse auf z.B. den Level FEINER gesetzt
 * werden, um so genauere Informationen über einen speziellen Programmteil zu
 * protokollieren.
 *
...
public class Debug {
	...
	/**
	 * Verwendung nur für Warnungen, die vom Programm zwar noch abgefangen
	 * werden können, aber unbedingt behoben werden müssen.
	 */
	public final static Level WARNING = Level.WARNING;

	/**
	 * Verwendung für Infoausgaben (z.B. Status des Programms, verwendete
	 * Startparameter etc.).
	 */
	public final static Level INFO = Level.INFO;

	/**
	 * Verwendung für Konfigurationsinformationen (z.B. angemeldete Objekte
	 * etc.).
	 */
	...
	/**
	 * (Default) Name des Wurzel-DebugLogger, wird auch als Bestandteil der
	 * Ausgabedateien verwendet
	 */
	...
	/**
	 * Pfad für Ausgabedateien. Der komplette Dateiname wird gebildet aus
	 * {@link #_debugFilePath}/debug/{@link #_rootName}.{@link #_debugFileNamePattern}._fileExtensionXYZ
	 * (z.B. "C:/root-%u-%g.xml") Kann über Aufrufparameter geändert werden.
	 */
	...
	/**
	 * Dateinamemuster für Ausgabedateien. Der komplette Dateiname wird gebildet
	 * aus
	 * {@link #_debugFilePath}/{@link #_rootName}.{@link #_debugFileNamePattern}._fileExtensionXYZ
	 * (z.B. "C:/root-%u-%g.xml") Kann über Aufrufparameter geändert werden.
	 */
	private static String _debugFileNamePattern = "-%u-%g";

	/**
	 * Dateiendung für XML-Dateien. Der komplette Dateiname wird gebildet aus
	 * {@link #_debugFilePath}/{@link #_rootName}.{@link #_debugFileNamePattern}.{@link #_fileExtensionXML}
	 * (z.B. "C:/root-%u-%g.log.xml") Kann über Aufrufparameter geändert werden.
	 */
	private static String _fileExtensionXML = ".log.xml";

	/**
	 * Dateiendung für Text-Dateien. Der komplette Dateiname wird gebildet aus
	 * {@link #_debugFilePath}/{@link #_rootName}.{@link #_debugFileNamePattern}.{@link #_fileExtensionText}
	 * (z.B. "C:/root-%u-%g.log.txt") Kann über Aufrufparameter geändert werden.
	 */
	private static String _fileExtensionText = ".log.txt";

	/**
	 * Dateiendung für HTML-Dateien. Der komplette Dateiname wird gebildet aus
	 * {@link #_debugFilePath}/{@link #_rootName}.{@link #_debugFileNamePattern}.{@link #_fileExtensionHTML}
	 * (z.B. "C:/root-%u-%g.log.html") Kann über Aufrufparameter geändert
	 * werden.
	 */
	private static String _fileExtensionHTML = ".log.html";

	/**
	 * Dateiendung für Excel-Dateien. Der komplette Dateiname wird gebildet aus
	 * {@link #_debugFilePath}/{@link #_rootName}.{@link #_debugFileNamePattern}.{@link #_fileExtensionExcel}
	 * (z.B. "C:/root-%u-%g.log.csv") Kann über Aufrufparameter geändert werden.
	 */
	private static String _fileExtensionExcel = ".log.csv";

	/**
	 * Ungefähre maximale Größe einer Ausgabedatei. Kann über Aufrufparameter
	 * geändert werden.
	 */
	 ...
	 * Kann über Aufrufparameter geändert werden.
	 */
	private static int _debugFileCount = 5;

	/**
	 * Legt fest ob an bereits existente Ausgabedateien angehängt werden darf,
	 * oder ob die alte Datei überschrieben wird. Kann über Aufrufparameter
	 * geändert werden.
	 */
	private static boolean _debugFileAppend = false;

	/**
	 * Aktueller DebugLevel für Dateiausgaben im XML-Format Kann über
	 * Aufrufparameter geändert werden.
	 */
	private static Level _debugLevelFileXML = OFF;

	/**
	 * Aktueller DebugLevel für Dateiausgaben im Text-Format Kann über
	 * Aufrufparameter geändert werden.
	 */
	private static Level _debugLevelFileText = OFF;

	/**
	 * Aktueller DebugLevel für Dateiausgaben im csv-Format Kann über
	 * Aufrufparameter geändert werden.
	 */
	private static Level _debugLevelFileExcel = OFF;

	/**
	 * Aktueller DebugLevel für Dateiausgaben im HTML-Format Kann über
	 * Aufrufparameter geändert werden.
	 */
	private static Level _debugLevelFileHTML = OFF;

	/**
	 * Aktueller DebugLevel für StdErr-Ausgaben im Text-Format Kann über
	 * Aufrufparameter geändert werden.
	 */
	 ...
	 * <ul>
	 * <li>Alle registrierten DebugLogger werden entfernt</li>
	 * <li>Es wird der Wurzel-DebugLogger initialisiert, der den Namen des
	 * Aufrufparameters erhält.</li>
	 * <li>Es wird der Dateipfad für alle dateibasierten Debugausgaben
	 * festgelegt</li>
	 * <li>Es werden folgende Ausgabekanäle und Ausgabeformate angemeldet und
	 * die Ausgabelevel auf Default eingestellt:</li>
	 * </ul>
	 * <ul>
	 * <li>Datei im XML-Format (*.xml), DebugLevel:INFO</li>
	 * <li>Datei mit Ausgaben als formatierter Text (*.txt),
	 * DebugLevel:INFO</li>
	 * <li>Datei im EXCEL-Format (*.csv), DebugLevel:OFF</li>
	 * <li>Datei im HTML-Format(*.htm), DebugLevel:OFF</li>
	 * <li>Ausgabe auf stderr als Text, DebugLevel:INFO</li>
	 * </ul>
	 * Die Ausgabekanäle/Ausgabeformate stehen anschliessend allen DebugLoggern
	 * automatisch zur Verfügung.
	 * <ul>
	 * <li>Auswertung der Aufrufparameter der Applikation</li>
	 * <li>Anmeldung der Telegramme zur Steuerung des DebugLoggers über den
	 * Datenverteiler</li>
	 * <li>Alle registrierten DebugLogger werden entfernt</li>
	 * </ul>
	 *
	 * @param argumentList
	 *            Liste der bei Applikationsstart übergebenen Parameter zur
	 *            Voreinstellung.
	 * @param rootName
	 *            Wurzel-Name aller verwalteter DebugLogger
	 *
	 * @since V 1.0
	 */
	public static void init(String rootName, ArgumentList argumentList) {
		...
		if (_rootLogger != null && !rootName.equals("DEFAULT-DEBUG") && !_rootName.equals("DEFAULT-DEBUG")) {
			boolean ignoreMultipleInits = System.getProperty("de.bsvrz.sys.funclib.debug.ignoreMultipleInits", "false")
					.equals("true");
			if (ignoreMultipleInits) {
				getLogger().warning(
						"Erneute Initialisierung des Debug-Loggers wird ignoriert, bisherige Einstellungen bleiben erhalten. (ignoriert: "
								+ rootName + ", bisher: " + _rootName + ")");
				return;
			} else {
				getLogger()
						.warning("Erneute Initialisierung des Debug-Loggers überschreibt bisherige Einstellung. (neu: "
								+ rootName + ", überschrieben: " + _rootName + ")");
			}
		}
		...
		try {
			...
			_debugLevelStdErrText = string2Level(
					argumentList.fetchArgument("-debugLevelStdErrText=WARNING").asString());
			...
			stackTracesSelection.add("ja").alias("wahr").alias("an").alias("true").alias("yes").alias("on").ignoreCase()
					.convertTo(1);
			stackTracesSelection.add("nein").alias("falsch").alias("aus").alias("false").alias("no").alias("off")
					.ignoreCase().convertTo(1);
			debugStdErrWithStackTraces = argumentList.fetchArgument("-debugStdErrTextStacktraces=auto")
					.asValueCase(stackTracesSelection).intValue();
		...
		try {
			...
			// Handler und Formatter erzeugen, aber nur, wenn mindestens ein
			// FileFormater ungleich OFF
			if (!((_debugLevelFileXML == OFF) && (_debugLevelFileText == OFF) && (_debugLevelFileHTML == OFF)
					&& (_debugLevelFileExcel == OFF))) {
				// System.err.println("Debug-Ordner anlegen: " +
				// directory.getAbsolutePath());
				// System.err.println("filename: " + fileName);
				directory.mkdir();
			}

			// Jeweils Handler und Formatter holen und verknüpfen, Handlerlevel
			// setzen und beim
			// Rootlogger registrieren.
			// Registrierung erfolgt nur, wenn Level ungleich OFF ist (Ausnahme:
			// Ausgabe auf StdErr erfolgt immer!
			...
			_formatterStdErrText = new DebugFormatterStdErrText(debugStdErrWithStackTraces == 1); // Formatierer
																									// für
																									// Textausgabe
																									// auf
																									// StdErr
			...
			if (!_debugLevelFileXML.equals(OFF)) {
				_handlerFileXML = new FileHandler(fileName + _fileExtensionXML, _debugFileSize, _debugFileCount,
						_debugFileAppend);
				_formatterXML = new DebugFormatterXML(); // Formatierer für
															// XML-Formatierung
															// holen
			...
			if (!_debugLevelFileText.equals(OFF)) {
				_handlerFileText = new FileHandler(fileName + _fileExtensionText, _debugFileSize, _debugFileCount,
						_debugFileAppend);
				_formatterFileText = new DebugFormatterFileText(); // Formatierer
																	// für
																	// Textausgabe
																	// auf Datei
																	// holen
			...
			if (!_debugLevelFileHTML.equals(OFF)) {
				_handlerFileHTML = new FileHandler(fileName + _fileExtensionHTML, _debugFileSize, _debugFileCount,
						_debugFileAppend);
				_formatterHTML = new DebugFormatterHTML(); // Formatierer für
															// HTML-Formatierung
															// holen
			...
			if (!_debugLevelFileExcel.equals(OFF)) {
				_handlerFileExcel = new FileHandler(fileName + _fileExtensionExcel, _debugFileSize, _debugFileCount,
						_debugFileAppend);
				_formatterExcel = new DebugFormatterExcel(); // Formatierer für
																// Excel-Formatierung
																// holen
	...
	private static Level string2Level(String s) {
		if (s.trim().toUpperCase().equals("ERROR"))
			return ERROR;
		if (s.trim().toUpperCase().equals("WARNING"))
			return WARNING;
		if (s.trim().toUpperCase().equals("CONFIG"))
			return CONFIG;
		if (s.trim().toUpperCase().equals("INFO"))
			return INFO;
		if (s.trim().toUpperCase().equals("FINE"))
			return FINE;
		if (s.trim().toUpperCase().equals("FINER"))
			return FINER;
		if (s.trim().toUpperCase().equals("FINEST"))
			return FINEST;
	...
	/**
	 * Debugausgabe interner Art (auf Level INFO) für Ausgabe der aktuellen
	 * Einstellungen oder bei Änderung der Einstellungen. Bei Ausgabe über
	 * <source>status</source> wird der DebugLevel nur für diese Ausgabe auf
	 * INFO gesetzt
	 *
	 * @param msg
	 *            Auszugebender Debugtext. Der Text kann auch mehrzeilig
	 *            formatiert sein. Neben dem Text werden noch folgende Ausgaben
	 *            automatisch mit ausgegeben, so dass diese nicht im Text
	 *            enthalten sein sollten. (Datum, Zeit, Fehlerlevel (FEHLER,
	 *            WARNUNG,...), Klasse des Aufrufers, lfdNr der Meldung,...)
	 */
	private void status(String msg) {
		Level _aktLevel = _logger.getLevel(); // aktuellen Level merken...
		_logger.setLevel(INFO); // ...für die nächste Ausgabe Level auf INFO
								// setzen...
	...
	public static String debugInfo() {
		...
		// sb.append(NEWLINE);
		// sb.append("Basiseinstellung des
		// Wurzel-DebugLoggers").append(NEWLINE);
		// sb.append("----------------------------------------------").append(NEWLINE);
		// sb.append("Name :").append(_rootLogger.getName()).append(NEWLINE);
		// sb.append("DebugLevel
		// :").append(_rootLogger.getLevel()).append(NEWLINE);
		// sb.append("Filter
		// :").append(_rootLogger.getFilter()).append(NEWLINE);
		// sb.append("ParentName
		// :").append(_rootLogger.getParent()).append(NEWLINE);
		// sb.append("RCS-Bundle
		// :").append(_rootLogger.getResourceBundle()).append(NEWLINE);
		// sb.append("RCS-Bundle-Name
		// :").append(_rootLogger.getResourceBundleName()).append(NEWLINE);

		return (sb.toString());
	}

	/**
	 * Gibt eine DebugLogger zurück, der automatisch in der Klassenhierachie
	 * unter dem WurzelLogger einsortiert wurde. Existiert der DebugLogger noch
	 * nicht, wird er erzeugt, ansonsten wird der bereits existierende
	 * DebugLogger mit dem angeforderten Namen zurückgegeben. Als Name wird
	 * automatisch gebildet aus "rootName.voll qualifizierter ClassName" z.B.
	 * "root.sys.funclib.Debug". Für jeden Logger kann der DebugLevel per
	 * DaV-Telegramm geändert werden.
	 *
	 * @return Logger Objekt, welches für die Debugausgeben und zur Steuerung
	 *         des Debuglevels verwendet wird.
	 */
	...
	/**
	 * TESTMETHODE: Setzt DebugLevel des entsprechenden Loggers. Setzt den Level
	 * des angebenen Loggers UND aller untergeordneten Logger auf den
	 * eingestellten Level.
	 * <p>
	 * Es wird dazu der Name des Loggers verwendet. Als untergeordnet gelten
	 * aller Logger, deren Namensanfang mit dem des angegebenen Loggers
	 * übereinstimmt.
	 *
	 * @param logger
	 *            Name des mit {@link #getLogger} erzeugter DebugLogger, dessen
	 *            Level geändert werden soll
	 * @param level
	 *            Neuer DebugLevel ({@link #ERROR}, {@link #WARNING},
	 *            {@link #INFO}, {@link #CONFIG}, {@link #FINE}, {@link #FINER},
	 *            {@link #FINEST}, {@link #ALL}, {@link #OFF}).
	 */
	...
	/**
	 * TESTMETHODE: Setzt DebugLevel des entsprechenden Ausgabekanals. Damit
	 * werden alle nur noch Ausgaben auf diesem Kanal mit dem eingestellten
	 * Level oder darüber ausgegeben, unabhängig davon ob einzelne Logger einen
	 * feineren Level eingestellt haben. Die Leveleinstellungen der einzelnen
	 * Logger werden aber nicht verändert.
	 *
	 * @param s
	 *            Ausgabekanal, dessem Level geändert werden soll (StdErr,
	 *            FileText, FileXML, FileHTML, FileExcel)
	 * @param level
	 *            Neuer DebugLevel ({@link #ERROR}, {@link #WARNING},
	 *            {@link #INFO}, {@link #CONFIG}, {@link #FINE}, {@link #FINER},
	 *            {@link #FINEST}, {@link #ALL}, {@link #OFF}).
	...
	public static void setHandlerLevel(String s, Level level) {
		if (s == "StdErr") {
			...
			_rootLogger.error("Levelumschaltung des Ausgabekanals [" + s + "] auf Level " + level
					+ " nicht möglich, da Level [" + s + "] nicht zulässig!");
		}
	}
	 ...
	 *            WARNUNG,...), Klasse des Aufrufers, lfdNr der Meldung,...)
	 * @param thrown
	 *            Zusätzliche Meldungen, die sich aus der übergebenen Exeption
	 *            ergibt.
	 */
	 ...
	 *            WARNUNG,...), Klasse des Aufrufers, lfdNr der Meldung,...)
	 * @param param
	 *            Zusätzliche Meldungen, die über die {@link #toString} Methode
	 *            an den übergebenen Objekten ausgewertet werden.
	 */
	 ...
	 *            WARNUNG,...), Klasse des Aufrufers, lfdNr der Meldung,...)
	 * @param param
	 *            Zusätzliche Meldung, die über die {@link #toString} Methode am
	 *            übergebenen Objekt ausgewertet werden.
	 */
	public void config(String msg, Object param) {
		_logger.log(CONFIG, msg, param);
	}
	 ...
	 * <p>
	 * Verwendung nur für "echte" Fehlerausgaben, die vom Programm nicht
	 * abgefangen werden oder die zu unkontrollierbaren Folgefehlern führen.
	 *
	 * @param msg
	 *            Auszugebender Debugtext. Der Text kann auch mehrzeilig
	 *            formatiert sein. Neben dem Text werden noch folgende Ausgaben
